...
struct WeekDetailView: View {
    ...
    var normalizedStats: [(SharkGender, Double)] {
        let total = Double(weekDates.count)
        return SharkGender.allCases.map { gender in
            let count = Double(weekStats[gender] ?? 0)
            return (gender, count / total)
        }
    }
        VStack(alignment: .leading, spacing: 12) {
            Text("Weekly Gender Breakdown")
                .font(.headline)
            ...
            GenderBarGraphView(dates: weekDates)
                .frame(height: 200)
        }
        .padding()
